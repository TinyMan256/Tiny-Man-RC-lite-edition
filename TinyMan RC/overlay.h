...
{
	float xPos = (GetSystemMetrics(SM_CXSCREEN) - rc.right) / 2;
	float yPos = (GetSystemMetrics(SM_CYSCREEN) - rc.bottom) / 2;
	auto vList = ImGui::GetOverlayDrawList();
	vList->AddLine({ xPos ,yPos - (aSize + 1) }, { xPos ,yPos + (aSize + 1) }, aColor, cfg.crosshairwidth);
	vList->AddLine({ xPos - (aSize + 1),yPos }, { xPos + (aSize + 1)  ,yPos }, aColor, cfg.crosshairwidth);
}
...
auto DrawBox(float x, float y, float w, float h, ImColor color, float thickness)-> VOID
{
	...
	DrawLine(ImVec2(x, y - 1.3f), ImVec2(x, y + h + 1.4f), color, thickness); // left
	DrawLine(ImVec2(x + w, y - 1.3f), ImVec2(x + w, y + h + 1.4f), color, thickness);  // right
...
auto DrawCorneredBox(float X, float Y, float W, float H, const ImU32& color, float thickness) -> VOID
{
	...
	vList->AddLine(ImVec2(X, Y + H - lineH), ImVec2(X, Y + H + (thickness / 2)), col, thickness);//top left
	vList->AddLine(ImVec2(X - thickness / 2, Y + H), ImVec2(X + lineW, Y + H), col, thickness);
	vList->AddLine(ImVec2(X + W - lineW, Y + H), ImVec2(X + W + thickness / 2, Y + H), col, thickness);//top right
	vList->AddLine(ImVec2(X + W, Y + H - lineH), ImVec2(X + W, Y + H + (thickness / 2)), col, thickness);
}
auto DrawBox2(float X, float Y, float W, float H, const ImU32& color, float thickness) -> VOID
{
	auto vList = ImGui::GetOverlayDrawList();
	float lineW = (W / 3);
	float lineH = (H / 3);
...
auto DrawBox3(float X, float Y, float W, float H, const ImU32& color, float thickness) -> VOID
{
	auto vList = ImGui::GetOverlayDrawList();
	float lineW = (W / 3);
	float lineH = (H / 3);
...
auto DrawBox4(float X, float Y, float W, float H, const ImU32& color, float thickness) -> VOID
{
	auto vList = ImGui::GetOverlayDrawList();
	float lineW = (W / 1.5);
	float lineH = (H / 1.5);
