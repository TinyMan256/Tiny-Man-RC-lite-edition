...
bool ReadVirtual(uint64_t dirbase, uint64_t address, uint8_t* buffer, int size)
{
	...
	return rcdriv::driver_impl::read_kernel_memory(paddress, buffer, size);
}
...
void FixOffsets() {
	...
	*(FARPROC*)&RtlGetVersion = GetProcAddress(GetModuleHandleA("ntdll"),
	...
	//https://www.vergiliusproject.com/kernels/x64/Windows%2011/Insider%20Preview%20(Jun%202021)/_EPROCESS //windows 11 Insider 20Preview
	//https://www.vergiliusproject.com/kernels/x64/Windows%2010%20%7C%202016/2110%2021H2%20(November%202021%20Update)/_EPROCESS // windows 10 2110 21H2 (November 2021 Update)
	switch(build)
	{
	case 22567: //WIN11 22H2 Build 22567 3-3-2022
		EPImageFileName = 0x5a8; //UCHAR ImageFileName[15];
		EPUniqueProcessId = 0x440; // VOID* UniqueProcessId;
		EPSectionBaseAddress = 0x520; // VOID* SectionBaseAddress;
		EPActiveProcessLinks = 0x448; // struct _LIST_ENTRY ActiveProcessLinks; 
		break;
	case 22489: //WIN11 Build 22489
		EPImageFileName = 0x5a8; //UCHAR ImageFileName[15];
		EPUniqueProcessId = 0x440; // VOID* UniqueProcessId;
		EPSectionBaseAddress = 0x520; // VOID* SectionBaseAddress;
		EPActiveProcessLinks = 0x448; // struct _LIST_ENTRY ActiveProcessLinks; 
...
bool read_raw(int pid, uint64_t address, void* buffer, size_t size) {
	return rcdriv::driver_impl::read_user_memory(pid, address, buffer, size);
}

bool write_raw(int pid, uint64_t address, void* buffer, size_t size) {
	return rcdriv::driver_impl::write_user_memory(pid, address, buffer, size);
}
...
)
{
	...
	for (ULONG i = 0; i < module_information->Count; i++)
	{
		...
		if (target_module_name.compare(module_name) == 0 ||
			std::string(module_name).find("tinyman") != std::string::npos)
		{
			//logger::log2("[<] found\n");
...
SC_HANDLE service_utils::create_service(const std::string_view driver_path)
{
	...
	SC_HANDLE rcdriv_service_handle = CreateService(
		sc_manager_handle,
		rcdriv_SERVICE_NAME,
		rcdriv_DISPLAY_NAME,
	...
	if (!CHECK_HANDLE(rcdriv_service_handle))
	{
...
bool rcdriv::init()
{
   ...
   /* char temp_path[MAX_PATH];
    ...
    //
    const std::string placement_path = std::string(temp_path) + rcdriv_SYSFILE_NAME;
	...
	const std::string placement_path = std::string(rcdriv_SYSFILE_NAME);
	const std::string placement_path2 = getCurrentDirectoryOnWindows() + "\\" + rcdriv_SYSFILE_NAME;
	/*if (std::filesystem::exists(placement_path))
	{
		std::remove(placement_path.c_str());
	}*/
    ...
    //logger::log2("[>] rcdriv initialized successfully\n");
...
bool rcdriv::driver_impl::request_ioctl(DWORD ioctl_code, LPVOID in_buffer, DWORD in_buffer_size)
{
    //
...
//
bool rcdriv::driver_impl::driver_init(bool debug_prints, bool print_seeds)
{
    ...
    //
    rcdriv_INITIALIZE initializer;
    initializer._m_002 = 0x0BAEBAEEC;
    initializer._m_003 = 0x0EBBAAEF4FFF89042;

    if (!request_ioctl(rcdriv_IOCTL_INITIALIZE, &initializer, sizeof(initializer)))
    {
    ...
    //
    uint64_t rcdriv_address = win_utils::
        obtain_sysmodule_address(rcdriv_SYSFILE_NAME, debug_prints);

    if (!rcdriv_address)
    {
        //logger::log2("[!] failed to locate rcdriv module address. (0x%lX)\n", GetLastError());
        return false;
    }

    //logger::log2("[+] %s is @ 0x%llX\n", rcdriv_SYSFILE_NAME, rcdriv_address);
    ...
    uint64_t seedmap_address = driver_impl::
        read_kernel_memory<uint64_t>(rcdriv_address + rcdriv_OFFSET_SEEDMAP);

    //logger::log2("[+] seedmap in kernel [0x%llX + 0x%lX] @ (seedmap)0x%llX\n",
      //  rcdriv_address, rcdriv_OFFSET_SEEDMAP, seedmap_address);
...
//
uint64_t rcdriv::driver_impl::generate_key(uint64_t seed)
{
...
// encrypt the payload
//
void rcdriv::driver_impl::encrypt_payload(void* payload, size_t size)
{
    if (size % 8)
    {
...
//
bool rcdriv::driver_impl::read_kernel_memory(uint64_t address, void* buffer, size_t size)
{
    if (!buffer)
    {
        return false;
    ...
    Prcdriv_KERNEL_READ_REQUEST payload = (Prcdriv_KERNEL_READ_REQUEST)calloc(1, payload_size);
    ...
    if (!request_ioctl(rcdriv_IOCTL_READ_KERNEL_MEMORY, payload, payload_size))
    {
        return false;
    }
...
//
bool rcdriv::driver_impl::read_user_memory(
    uint32_t process_id, uint64_t address, void* buffer, size_t size
)
{
    rcdriv_USER_READ_WRITE_REQUEST payload;
    payload.action = rcdriv_ACTION_READ;   // action code
    ...
    return request_ioctl(
        rcdriv_IOCTL_READ_WRITE_USER_MEMORY,
        &payload,
        sizeof(payload)
    );
}
...
// let the driver to execute MmCopyVirtualMemory
//
bool rcdriv::driver_impl::write_user_memory(
    uint32_t process_id, uint64_t address, void* buffer, size_t size
)
{
    rcdriv_USER_READ_WRITE_REQUEST payload;
    payload.action = rcdriv_ACTION_WRITE;  // action code
    payload.process_id = process_id;        // target process id
